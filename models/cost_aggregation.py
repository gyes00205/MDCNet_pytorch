...
class CostAggregation_2d(nn.Module):
    ...
    def forward(self, x):
        ...
        conv2_up = self.conv2_up(torch.cat([conv4_down, pad(conv4_down, conv1_up)], dim=1))
        conv3_up = self.conv3_up(torch.cat([conv3_down, pad(conv3_down, conv2_up)], dim=1))
        conv4_up = self.conv4_up(torch.cat([conv2_down, pad(conv2_down, self.deconv1(conv3_up))], dim=1))
        post_conv = self.post_conv(torch.cat([conv1_down, pad(conv1_down, self.deconv2(conv4_up))], dim=1))
