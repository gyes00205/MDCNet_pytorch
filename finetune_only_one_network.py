...
parser.add_argument('--save_path', type=str, default='ablation_study/kitti15_only_2D',
...
parser.add_argument('--pretrained', type=str, default='ablation_study/pretrained_only_2D_network/checkpoint23.tar',
...
def train(dataloader, model, optimizer, log, epoch=0):
    ...
    for idx in range(stages):
        writer.add_scalar(f'KITTI{args.datatype} Train Epoch/loss/stage {idx}', losses[idx].avg, epoch)
...
def test(dataloader, model, log, epoch=0):
    ...
    for idx in range(stages):
        writer.add_scalar(f'KITTI{args.datatype} Val Epoch/disparity map D1/stage {idx}', D1s[idx].avg, epoch)
    
    info_str = ', '.join(['Stage {}={:.4f}'.format(x, D1s[x].avg) for x in range(stages)])
    log.info('Average test 3-Pixel Error of disparity map = ' + info_str)
